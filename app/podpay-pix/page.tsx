...
export default function PodPayPixPage() {
  ...
  useEffect(() => {
    // Get PodPay transaction data from localStorage
    const storedTransactionData = localStorage.getItem("podpayTransaction")
    if (storedTransactionData) {
      const data = JSON.parse(storedTransactionData)
      console.log("[v0] PodPay transaction data loaded:", data)
      setTransaction(data)
    }
    setIsLoading(false)
  }, [])
  ...
  const copyPixCode = async () => {
    const pixCode = transaction?.pix?.qrCodeText
    if (pixCode) {
      try {
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4 max-w-md mx-auto w-full">
        {paymentApproved ? (
          ...
          <>
            ...
            <p className="text-sm text-gray-600 mb-4 text-center">Powered by PodPay</p>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-lg mb-6 w-full max-w-sm">
              <div className="flex justify-center mb-4">
                {transaction.pix?.qrCode ? (
                  <img src={transaction.pix.qrCode || "/placeholder.svg"} alt="QR Code PIX" className="w-48 h-48" />
                ) : transaction.pix?.qrCodeText ? (
                  <img
                    src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(transaction.pix.qrCodeText)}`}
                    alt="QR Code PIX"
                    className="w-48 h-48"
                  />
                ) : (
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-gray-100 p-3 rounded-lg">
                    <p className="text-xs font-mono break-all text-gray-800">
                      {transaction.pix.qrCodeText.substring(0, 50)}...
                    </p>
                  </div>
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm mb-4 w-full max-w-sm">
              <div className="text-xs text-gray-600 space-y-1">
                ...
                {transaction.pix?.expiresAt && (
                  <p>
                    <strong>Expira em:</strong> {new Date(transaction.pix.expiresAt).toLocaleString("pt-BR")}
                  </p>
                )}
              </div>
            </div>
